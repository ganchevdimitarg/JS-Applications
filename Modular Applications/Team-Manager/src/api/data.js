...
export async function getMyTeams() {
    ...
    const [teamsCreated, teamMember] = await Promise.all([
        ...
        api.get(host + `/data/teams?where=_ownerId%3D%22${userId}%22%20AND%20status%3D%22member%22&load=team%3DteamId%3Ateams`)
    ]);
...
export async function createTeam(team) {
    ...
    const request = await reqToJoin(result._id);
    const asdas = await approveMembership(request);
    return result;
}
...
export async function reqToJoin(teamId) {
    const body = {teamId};
    return await api.post(host + '/data/members', body);
}

export async function getReqsByTeamId(teamId) {
