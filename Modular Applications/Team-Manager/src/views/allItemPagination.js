...
import {getAllItem, getCollectionSize} from "../api/data.js";


const allItemsWithPaginationTemp = (data, page, pages) => html`
...
export async function createAllItemWithPaginationPage(ctx) {
    const page = Number(ctx.querystring.split('=')[1]) || 1;

    const count = await getCollectionSize();
    const pages = Math.ceil(count / 3);
    const cars = await getAllItem(page);

    ctx.render(getAllItem(cars, page, pages));
}